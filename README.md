...
- **链接解析与播放**：在启动页按 `Ctrl+V` 粘贴链接，可解析 B站视频与番剧（ep / ss / md）、YouTube 视频，以及 B站、抖音、斗鱼、虎牙直播的网页链接
- **直播画质与弹幕**：自动选择可用的最高画质，也可手动切换清晰度与线路；B站、斗鱼、虎牙直播支持弹幕，斗鱼登录后可使用账号开放的高码率档位
- **在线进度预览**：可定位的网络视频与 AList/OpenList/WebDAV 文件支持进度条画面预览
