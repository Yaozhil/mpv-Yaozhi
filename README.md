...
面向 Windows mpv 的自制 UI 整合包<br>基于 [dyphire/mpv-config](https://github.com/dyphire/mpv-config) 的配置结构继续整理<br>并加入个人维护的 uosc 布局、快捷键菜单、弹幕、片头片尾标记、AList/OpenList 网盘入口、起播格式徽章、音乐模式、HDR 图形特效字幕增强和中文统计信息体验
...
- 原创音乐模式：支持列表循环、随机防重复、单曲循环，音乐文件最小化不暂停
- 播放核心重构：自编译 mpv 核心，加入 HDR 图形字幕补丁，并恢复 Windows Schannel 网络兼容
