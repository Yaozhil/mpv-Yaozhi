...
- **链接解析与播放**：在启动页按 `Ctrl+V` 粘贴链接，可播放 B站视频与番剧（ep / ss / md）、YouTube 视频，以及 B站、抖音、斗鱼、虎牙直播。
- **直播画质与弹幕**：自动选择可用的高画质，也可手动切换清晰度与线路；B站、斗鱼、虎牙直播支持弹幕，斗鱼登录后可使用账号开放的高码率档位。
- **在线进度预览**：可定位的网络视频与 AList/OpenList/WebDAV 文件支持进度条画面预览；慢速来源预览超时后会自动停用，不拖慢当前播放。
...
<details>
<summary>查看完整起播格式标签</summary>

<br>

![彩色版起播格式标签](docs/images/startup-format-badges-color-overview.png)

![透明白色版起播格式标签](docs/images/startup-format-badges-white-overview.png)
...
如果 mpv-Yaozhi 有幸改善了你的播放体验，非常欢迎在能力范围内支持。

你的支持可以让我更好的 持续维护与开发本项目，总之非常感谢。
